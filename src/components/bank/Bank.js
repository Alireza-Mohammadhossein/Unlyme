...
const Bank = () => {
  const [
    dateSelectionPopupIsShowedForDateFrom,
    setDateSelectionPopupIsShowedForDateFrom,
  ] = useState(false);
  const [
    dateSelectionPopupIsShowedForDateTo,
    setDateSelectionPopupIsShowedForDateTo,
  ] = useState(false);

  const [dateFrom, setDateFrom] = useState(null);
  const [dateTo, setDateTo] = useState(null);

  const [typeOfTransactionFilter, setTypeOfTransactionFilter] = useState(
    TYPE_OF_TRANSACTION_FILTER_0
  );
  ...
  const accountsHistoryFiltered =
    appliedFilter != null
      ? accountsHistory.filter(
          (item) =>
            (appliedFilter.accountsFilter.length === 0 ||
              appliedFilter.accountsFilter.includes(item.accountId)) &&
            (appliedFilter.dateFrom !== null
              ? item.created_at >= appliedFilter.dateFrom.unix()
              : true) &&
            (appliedFilter.dateTo !== null
              ? item.created_at <= appliedFilter.dateTo.unix()
              : true) &&
            (appliedFilter.typeOfTransactionFilter ===
            TYPE_OF_TRANSACTION_FILTER_1
              ? item.amount > 0
              : true) &&
            (appliedFilter.typeOfTransactionFilter ===
            TYPE_OF_TRANSACTION_FILTER_2
              ? item.amount < 0
              : true)
        )
      : accountsHistory;
              ...
              content={accounts.map((card, index) => {
                ...
                return (
                  <>
                    <div
                      key={card.id}
                      className="services__bank_card services__bank-page_card"
                    >
                      <div
                        className={`services__bank_card-currency services__bank_card-currency--${
                          colors[index % 3]
                        }`}
                      >
                        {card.currency}
                      </div>
                      <div className="services__bank_balance">
                        {card.balance}
                      </div>
                      <div className="services__bank_balance-text">Balance</div>
                      <div className="services__bank_card-number">
                        {card.cardNumber}
                      </div>
                    </div>
                    {index !== accounts.length - 1 && (
                      <div className="services__bank-page_card-separator" />
                    )}
                  </>
                );
              })}
            ...
            <CloudPage
              ...
              content={
                <div className="row">
                  <div className="col-7">
                    {_.map(
                      _.groupBy(accountsHistoryFiltered, (item) =>
                        moment.unix(item.created_at).format("DD.MM.Y")
                      ),
                      (data, date) => {
                        return (
                          <div
                            key={date}
                            className="services__bank_history-section"
                          >
                            <div className="services__bank_history-section-date">
                              {date}
                            </div>
                            {data.map((item, index) => (
                              <React.Fragment key={item.id}>
                                <div className="services__bank_history-item">
                                  <div>
                                    <div className="services__bank_history-item-title">
                                      {item.title}
                                    </div>
                                    <div className="services__bank_history-item-subtitle">
                                      {item.subtitle}
                                    </div>
                                  </div>
                                  <div
                                ...
                                {index !== data.length - 1 && (
                                  <div className="services__bank_history-item-separator" />
                                )}
                              </React.Fragment>
                            ))}
                          </div>
                  ...
                  <div className="col-5" style={{ position: "relative" }}>
                    ...
                    <div className="cloud-page__header">
                      <div className="cloud-page__header_icon-container cloud-page__header_icon-container--orange">
                        <i
                          className="menu__row_icon material-icons"
                          style={{ color: "white" }}
                        >
                          filter_list
                        </i>
                      </div>
                      <div className="cloud-page__header_title">Filters</div>
                    </div>
                    <div className="cloud-page__content">
                      <div className="site-input-label">Accounts</div>
                      <CustomSelect
                        onChange={(v) =>
                          setAccountsFilter(v.map((o) => o.value))
                        }
                        value={_.filter(accountsOptions, (option) =>
                          accountsFilter.includes(option.value)
                        )}
                        placeholder="Select"
                        isMulti
                        options={accountsOptions}
                      />
                      <div className="site-input-label mt-2">
                        Type of transaction
                      </div>
                      <SegmentedControl
                        ...
                        options={[
                          ...
                          {
                            value: TYPE_OF_TRANSACTION_FILTER_1,
                            label: "Incoming",
                          },
                          {
                            value: TYPE_OF_TRANSACTION_FILTER_2,
                            label: "Outgoing",
                          },
                        ]}
                      ...
                      <div className="row">
                        <div className="col-6 services__bank-page_date-col">
                          ...
                          <input
                            ...
                            onFocus={() =>
                              setDateSelectionPopupIsShowedForDateFrom(true)
                            }
                            placeholder=""
                          />
                        </div>
                        <div className="col-6 services__bank-page_date-col">
                          ...
                          <input
                            ...
                            onFocus={() =>
                              setDateSelectionPopupIsShowedForDateTo(true)
                            }
                            placeholder=""
                          />
                        </div>
                      ...
                      <button
                        className="btn btn--transparent-blue mt-2"
                        onClick={applyFilter}
                      >
                        Apply
                      </button>
